-- P1 cloud hydration: adds the columns CloudAttemptSnapshot,
-- CloudSkillMasterySnapshot, and CloudExerciseReviewSnapshot need that the
-- existing schema does not yet persist, backfills historical rows, extends
-- record_exercise_attempt() to maintain them going forward, and adds the
-- three stable-cursor indexes the AttemptHydrationCursor /
-- MasteryHydrationCursor / ReviewHydrationCursor pagination contracts rely
-- on.

-- === exercise_attempts: performance_quality, practice_context ===

alter table public.exercise_attempts
  add column performance_quality smallint,
  add column practice_context text;

-- Backfill using the same average-score bands as
-- src/domain/scoring/scoring-config.ts's PERFORMANCE_QUALITY_BANDS
-- (speed 0.5 / accuracy 0.5). Historical speed_score can be null (it was
-- never required not-null); treated as 0, matching the client's own
-- calculateSpeedScore(), which returns 0 for any incomplete attempt.
update public.exercise_attempts
set
  performance_quality = case
    when (coalesce(speed_score, 0) * 0.5 + accuracy_score * 0.5) >= 90 then 5
    when (coalesce(speed_score, 0) * 0.5 + accuracy_score * 0.5) >= 75 then 4
    when (coalesce(speed_score, 0) * 0.5 + accuracy_score * 0.5) >= 50 then 3
    when (coalesce(speed_score, 0) * 0.5 + accuracy_score * 0.5) >= 25 then 2
    when (coalesce(speed_score, 0) * 0.5 + accuracy_score * 0.5) >= 1 then 1
    else 0
  end,
  practice_context = 'different_exercise'
where performance_quality is null;
...
alter table public.exercise_attempts
  add constraint exercise_attempts_performance_quality_check
    check (performance_quality between 0 and 5),
  add constraint exercise_attempts_practice_context_check
    check (
      practice_context in (
...
-- === user_skill_mastery: unique_exercise_ids, unhinted-success cursors ===
...
with mastery_backfill as (
  select
    ...
    coalesce(
      ...
      ) filter (where exercise_attempts.completed),
      '{}'
    ) as unique_exercise_ids,
    min(exercise_attempts.completed_at) filter (
      where exercise_attempts.completed
        and exercise_attempts.hint_level_used = 0
    ) as first_unhinted_success_at,
    max(exercise_attempts.completed_at) filter (
      where exercise_attempts.completed
        and exercise_attempts.hint_level_used = 0
    ) as latest_unhinted_success_at
  from public.exercise_attempts
  join public.exercise_skills
    on exercise_skills.exercise_id = exercise_attempts.exercise_id
  group by exercise_skills.skill_id, exercise_attempts.user_id
)
update public.user_skill_mastery as mastery
set
  unique_exercise_ids = coalesce(
    mastery_backfill.unique_exercise_ids,
    '{}'
  ),
  first_unhinted_success_at = mastery_backfill.first_unhinted_success_at,
  latest_unhinted_success_at = mastery_backfill.latest_unhinted_success_at
from mastery_backfill
where mastery.user_id = mastery_backfill.user_id
  and mastery.skill_id = mastery_backfill.skill_id;

-- === user_review_items: mastery_level, last_performance_quality, last_attempt_at ===
...
with review_backfill as (
  select
    review.user_id,
    review.exercise_id,
    coalesce(mastery.mastery_level, 0) as mastery_level,
    latest_attempt.performance_quality as last_performance_quality,
    coalesce(
  ...
  left join public.user_skill_mastery as mastery
    on mastery.user_id = review.user_id
    and mastery.skill_id = review.skill_id
  left join lateral (
    select
      exercise_attempts.performance_quality,
      exercise_attempts.completed_at,
      exercise_attempts.started_at
    from public.exercise_attempts
    where exercise_attempts.user_id = review.user_id
      and exercise_attempts.exercise_id = review.exercise_id
    order by
      coalesce(exercise_attempts.completed_at, exercise_attempts.started_at)
        desc
    limit 1
  ) as latest_attempt on true
)
update public.user_review_items as review
set
  mastery_level = review_backfill.mastery_level,
  last_performance_quality = coalesce(
    review_backfill.last_performance_quality,
    0
  ),
  last_attempt_at = review_backfill.last_attempt_at
from review_backfill
where review.user_id = review_backfill.user_id
  and review.exercise_id = review_backfill.exercise_id;
...
alter table public.user_review_items
  add constraint user_review_items_mastery_level_check
    check (mastery_level between 0 and 5),
  add constraint user_review_items_last_performance_quality_check
    check (last_performance_quality between 0 and 5);

-- === stable pagination cursor indexes ===
...
-- === record_exercise_attempt(): persist the new hydration fields ===
...
declare
  ...
  v_unique_exercise_ids uuid[];
  v_previous_first_unhinted_success_at timestamptz;
  v_previous_latest_unhinted_success_at timestamptz;
  v_first_unhinted_success_at timestamptz;
  v_latest_unhinted_success_at timestamptz;
...
begin
  ...
  loop
    ...
    select
      ...
      user_skill_mastery.first_unhinted_success_at,
      user_skill_mastery.latest_unhinted_success_at
    into
      v_previous_level,
      v_previous_score,
      ...
      v_average_accuracy,
      v_average_hint,
      v_previous_first_unhinted_success_at,
      v_previous_latest_unhinted_success_at
    from public.user_skill_mastery
    where user_skill_mastery.user_id = v_user_id
    ...
    -- Recomputed from exercise_attempts (the same source that already fed
    -- the count), not merged incrementally: a pure recompute is trivially
    -- correct under idempotent retry, since a duplicate call never reaches
    -- this point (the on-conflict-do-nothing insert + early return above
    -- already guard it).
    select
      count(distinct exercise_attempts.exercise_id)::integer,
      coalesce(
        array_agg(
          distinct exercise_attempts.exercise_id
          order by exercise_attempts.exercise_id
    ...
    into v_unique_exercises, v_unique_exercise_ids
    from public.exercise_attempts
    join public.exercise_skills
      on exercise_skills.exercise_id = exercise_attempts.exercise_id
    where exercise_attempts.user_id = v_user_id
    ...
    -- Only a fresh completed, unhinted success ever moves these cursors: an
    -- incomplete or hinted attempt must leave both timestamps untouched,
    -- and the first one is set exactly once, never overwritten afterward.
    if v_completed and v_hint_level = 0 then
      v_first_unhinted_success_at := coalesce(
        v_previous_first_unhinted_success_at,
        coalesce(v_completed_at, v_started_at)
      );
      v_latest_unhinted_success_at := greatest(
        coalesce(
          v_previous_latest_unhinted_success_at,
          coalesce(v_completed_at, v_started_at)
        ),
      ...
      v_first_unhinted_success_at := v_previous_first_unhinted_success_at;
      v_latest_unhinted_success_at := v_previous_latest_unhinted_success_at;
    end if;
