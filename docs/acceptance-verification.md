...
| AC-054 | 本分支證據：`keyboard-display.test.ts` 驗證實體按鍵格式化與單獨修飾鍵過濾；`scoring-feedback.spec.ts`（`keeps a physical keypress that lands immediately before a reload, with the same Attempt identity`）驗證按鍵事件會增加並持久化 `keystrokeCount`，重新載入後仍保留於同一 Attempt | PR #1 待補證據：`RecentKeypresses.vue`／對應元件測試與 `p1-editor-settings.spec.ts` 必須驗證最近 8 個顯示項目、`showKeypresses` 顯示控制、Restart／Retry／下一題／Resume 清空，以及顯示佇列不進入 `AttemptDraft` |
